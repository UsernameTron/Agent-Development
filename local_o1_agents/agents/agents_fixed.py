...
from config.config import CEO_MODEL, FAST_MODEL, EXECUTOR_MODEL_ORIGINAL, EXECUTOR_MODEL_DISTILLED, USE_DISTILLED_EXECUTOR
import time
import torch
...
# Helper functions for FAISS operations to avoid type checking errors
def faiss_add(index, vectors):
    """Helper function to add vectors to a FAISS index"""
    contiguous_vectors = np.ascontiguousarray(vectors)
    # Use the underlying add method with proper typing
    index.add(contiguous_vectors)
    return index

def faiss_search(index, query, k):
    """Helper function to search a FAISS index"""
    contiguous_query = np.ascontiguousarray(query)
    # Use the underlying search method with proper typing
...
class ImageAgent:
    def __init__(self, device=None):
        ...
        self.model = self.model.to(self.device)  # Fixed: assign model.to(device) back to model
        self.processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch16")
    
    def embed_image(self, image_path):
        image = Image.open(image_path).convert("RGB")
        inputs = self.processor(images=image, return_tensors="pt")
        # Move tensors in the inputs dictionary to the device
...
class PerformanceProfilerAgent:
    ...
    def profile_code(self, file_path, func_name=None):
        ...
        try:
            ...
            # Memory profiling without using tuple
            # Use a simpler approach to avoid type issues
            mem_usage_start = memory_usage()[0]
            run()  # Run again to measure memory
            mem_usage_end = memory_usage()[0]
            mem_profile = [mem_usage_start, mem_usage_end]
